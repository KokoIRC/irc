    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
            ...
            case 'err_nicknameinuse':
                ...
                self._nick = self.opt.nick + self.opt.nickMod;
                self._updateMaxLineLength();
                break;
            case 'PING':
            ...
            case 'NOTICE':
                ...
                if (self.opt.debug && to == self._nick)
            ...
            case 'PRIVMSG':
                ...
                if (to == self._nick) self.emit('pm', from, text, message);
...
Client.prototype.connect = function(retryCount, callback) {
    ...
    self.conn.addListener('data', function(chunk) {
        ...
        var lines = self.decode(buffer).toString().split(lineDelimiter);
...
Client.prototype.send = function(command) {
    ...
    if (!this.conn.requestedDisconnect) {
        ...
        var encoding = this.opt.encoding ? this.opt.encoding : Client.defaultEncoding;
        this.conn.write(this.encode(message), encoding);
    }
};

Client.prototype.raw = function (command, args) {
  this.send.apply(this, [command].concat(args));
};
Client.prototype.quote = Client.prototype.raw;
...
Client.prototype.mode = function(channel, mode, nick) {
  this.send('MODE', channel, mode, nick);
};

Client.prototype.kick = function(channel, nick, comment) {
  this.send('KICK', channel, nick, comment);
};

Client.prototype.kickban = function(channel, nick, comment) {
  this.send('KICK', channel, nick, comment);
  this.mode(channel, '+b', nick);
};

Client.prototype.ban = function(channel, nick) {
  this.mode(channel, '+b', nick);
};

Client.prototype.unban = function(channel, nick) {
  this.mode(channel, '-b', nick);
};
...
Client.defaultEncoding = 'UTF-8';
Client.prototype.decode = function(str) {
    var self = this;

    if (self.opt.encoding && self.opt.encoding !== Client.defaultEncoding) {
        return iconv.decode(str, self.opt.encoding);
    } else {
        return str;
    }
};
...
Client.prototype._updateMaxLineLength = function() {
    ...
    this.maxLineLength = 497 - this._nick.length - this.hostMask.length;
};
